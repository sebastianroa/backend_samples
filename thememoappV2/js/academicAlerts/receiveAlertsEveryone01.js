...
xhr.onreadystatechange = function() {
	if(xhr.readyState === 4 && xhr.status == 200) {
		...
		for(var i = 0; i < result.length; i++) {
			...
			statusHTML += '<a style= "display: block" href=receiveAlertsEveryone02.html?id='+result[i].task_identification+'>'+result[i].class_instructor+ ' , ' + result[i].university_name + '</a>';
			statusHTML += " ";
			statusHTML += '</br>';
